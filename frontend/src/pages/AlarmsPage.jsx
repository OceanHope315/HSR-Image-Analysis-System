...
import { alarmStatusLabel, formatDateTime, objectId } from '../utils/formatters.js';
...
export default function AlarmsPage() {
  ...
  const closeModals = () => { setAction(null); setModalError(''); };
  ...
  return (
    <div>
      <PageHeader title="报警中心" description="确认并跟踪高风险检测报警的人工处置过程" actions={<button className="button button--secondary" type="button" onClick={loadAlarms}>刷新报警</button>} />
            ...
            return (
              <article className={`alarm-card alarm-card--${alarm.level}`} key={alarm._id}>
                ...
                <div className="alarm-card-main">
                  ...
                  <div className="alarm-meta"><span>处理人员：<strong>安检员</strong></span><span>处置备注：<strong>{alarm.handlingNote || '暂无'}</strong></span></div>
                </div>
                <div className="alarm-card-actions">
                  ...
                  {nextStatuses.map((nextStatus) => <button key={nextStatus} type="button" className={`button button--small${nextStatus === 'ignored' ? ' button--ghost' : ' button--primary'}`} onClick={() => { setAction({ alarm, nextStatus }); setModalError(''); }}>{statusActionNames[nextStatus]}</button>)}
                </div>
              </article>
            );
          })}
